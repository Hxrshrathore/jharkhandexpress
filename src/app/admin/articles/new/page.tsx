...
import { addFeedItem, FormState } from './actions';
import { Plus, CheckCircle, AlertCircle, FileText, Loader2, Wand2, RefreshCw, UploadCloud, Check, ArrowLeft, Image as ImageIcon, Video, Code, Eye, X, Bell } from 'lucide-react';
...
export default function AdminPage() {
  const [state, formAction, isPending] = useActionState(addFeedItem, initialState);
  ...
  useEffect(() => {
    fetch('/api/admin/ads/active-count')
      .then(res => res.json())
      .then(data => setActiveAdCount(data.count || 0))
      .catch(console.error);
  }, []);
  // AI Formatted fields
  ...
  const [isPushBroadcast, setIsPushBroadcast] = useState(false);
  ...
  const handleBreakingChange = (checked: boolean) => {
    ...
    if (checked) {
      if (!categories.toLowerCase().includes('breaking news')) {
        setCategories(prev => prev ? prev + ', Breaking News' : 'Breaking News');
      }
    } else {
      setCategories(prev => prev.replace(/,?\s*breaking news/gi, '').replace(/^,\s*/, '').trim());
    }
  };
  ...
  useEffect(() => {
    if (state.success) {
      toast.success(state.message || 'ARTICLE ADDED');
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      setSelectedFiles(prev => {
        setFeaturedIndex(prev.length); // Auto select the newly uploaded image
        return [...prev, ...files];
      });
    ...
    if (e.target.files) {
      ...
      if (files.length > 0) {
        try {
          ...
          setSelectedFiles(prev => {
            if (prev.length === 0) setFeaturedIndex(0);
  ...
  const processWithAI = async () => {
    ...
    try {
      sendGAEvent('event', 'ai_formatting_started', {
        content_length: rawText.length
      });

      const mediaCount = uploaderRef.current ? uploaderRef.current.getTotalMediaCount() : 0;
      const requiredInserts = mediaCount + activeAdCount;
      const targetParagraphCount = requiredInserts > 0 ? requiredInserts + 1 : 4; // Sandwich everything or default to 4
      ...
      if (data.content_blocks && Array.isArray(data.content_blocks)) {
        ...
        // Generate the placeholders to weave
      ...
      setHtmlViewMode('preview'); // Automatically switch to preview tab
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (expiresDate) {
      formData.append('expires_at', expiresDate.toISOString());
    }
    ...
    for (let i = 0; i < compressedImageFiles.length; i++) {
      ...
      try {
        ...
        if (data.url) {
          ...
          if (i === finalFeaturedIndex) {
            finalFeaturedImage = data.url;
          }
        }
      } catch (e) {
    ...
    formData.append('gallery', JSON.stringify(uploadedImageUrls));
    
    // No videos are uploaded to R2.
    // We mandatorily upload the featured video to YouTube.
    if (selectedVideoFiles.length > 0 && selectedVideoFiles[featuredVideoIndex]) {
      setYoutubeUploading(true);
      setYoutubeUploadProgress(0);
      try {
        const videoFile = selectedVideoFiles[featuredVideoIndex];
        const resUrl = await fetch('/api/youtube/upload', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ title, description: excerpt, tags: tags.split(',').map(t => t.trim()) })
        });
        ...
        if (urlData.uploadUrl) {
          ...
          if (uploadRes.ok && uploadRes.data && uploadRes.data.id) {
            ...
            formData.append('featured_video_id', ytId);
            formData.append('youtube_video_ids', JSON.stringify([ytId]));
          }
        }
      } catch (err) {
    ...
    sendGAEvent('event', 'article_publish_started', {
      title: title,
      category_count: categories.split(',').length
    });
  ...
  return (
    <div className="relative min-h-screen bg-transparent pb-24 overflow-x-hidden">
      {/* Background ambient glow - retained for "Intelligence Core" vibe */}
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 mt-8 md:mt-12 relative">
        <form onSubmit={handleSubmit}>
          {isPreviewMode ? (
            /* FULL PAGE ARTICLE PREVIEW MODE */
            <div className="bg-background rounded-2xl shadow-xl overflow-hidden border border-border min-h-150 pb-32 animate-in fade-in duration-500">
              ...
              {(previewUrls.length > 0 || videoPreviewUrls.length > 0) && (
                <div className="w-full aspect-video md:aspect-21/9 bg-muted relative">
                  {videoPreviewUrls.length > 0 ? (
                    <video src={videoPreviewUrls[featuredVideoIndex || 0]} className="w-full h-full object-cover" />
                  ) : (
                    <img src={previewUrls[featuredIndex || 0]} className="w-full h-full object-cover" />
                  )}
                </div>
              )}
              ...
              <div className="fixed bottom-24 md:bottom-8 left-1/2 -translate-x-1/2 bg-card/90 backdrop-blur-xl border border-border shadow-2xl rounded-full px-2 sm:px-4 py-2 sm:py-3 flex items-center gap-2 sm:gap-3 z-50 animate-in slide-in-from-bottom-10 w-max max-w-[95vw] justify-center">
                ...
                <Button 
                  ...
                  className="rounded-full px-5 sm:px-8 shadow-lg shadow-primary/20"
                >
                  {isPending || isUploading ? (
                    <><Loader2 className="w-4 h-4 sm:mr-2 animate-spin" /> <span className="hidden sm:inline">PUBLISHING...</span><span className="sm:hidden ml-1">WAIT...</span></>
                  ) : (
                    <><CheckCircle className="w-4 h-4 sm:mr-2" /> <span className="hidden sm:inline">POST ARTICLE</span><span className="sm:hidden ml-1">POST</span></>
                  )}
                </Button>
              </div>
            </div>
          ) : (
            /* STANDARD FORM EDIT MODE */
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 xl:gap-12 animate-in fade-in duration-500">
              ...
              <div className="lg:col-span-5 space-y-8">
                ...
                <Card className={`relative overflow-hidden group shadow-lg transition-all ${isBreaking ? 'border-red-500 ring-2 ring-red-500/20' : ''}`}>
                ...
                <Card className="relative overflow-hidden group shadow-lg">
                  ...
                  <CardContent className="pt-6 space-y-6">
                    <div className="relative border border-dashed border-border hover:border-blue-500/50 hover:bg-blue-500/5 rounded-xl p-8 text-center cursor-pointer transition-all group">
                      ...
                      <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest group-hover:text-foreground">Drop Images</p>
                    </div>
                    ...
                    <div className="relative border border-dashed border-border hover:border-purple-500/50 hover:bg-purple-500/5 rounded-xl p-8 text-center cursor-pointer transition-all group mt-4">
                      ...
                      <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest group-hover:text-foreground">Drop Videos</p>
                    </div>
                ...
                {/* Step 2: Content & AI */}
                <Card className="relative overflow-hidden group shadow-lg">
                  <div className="absolute top-0 right-0 w-32 h-32 bg-orange-500/5 rounded-full blur-3xl pointer-events-none group-hover:bg-orange-500/10 transition-all duration-700"></div>
                  <CardHeader className="border-b border-border pb-4">
                    <CardTitle className="flex items-center gap-3 text-sm font-bold tracking-widest uppercase">
                  ...
                  <CardContent className="pt-6 space-y-4">
                    ...
                    <Textarea
                      ...
                      rows={6}
                      placeholder="Paste unformatted intel here..."
                      className="w-full bg-muted/30 resize-none transition-all focus:shadow-[0_0_15px_rgba(249,115,22,0.1)]"
                    />
                    
                    <Button
                      type="button"
                      onClick={processWithAI}
                      disabled={aiProcessing || !rawText}
                      className={`w-full py-6 flex justify-center items-center gap-2 ${
                        !aiProcessing && rawText ? 'bg-orange-500 hover:bg-orange-600 text-white shadow-md' : ''
                      }`}
                    >
                      {aiProcessing ? <Loader2 className="w-4 h-4 animate-spin" /> : <Wand2 className="w-4 h-4" />}
                      {aiProcessing ? 'Processing...' : 'Format with AI'}
                    </Button>
                  </CardContent>
                </Card>
                ...
                <Card className="shadow-lg">
                  <CardHeader>
                    <CardTitle className="text-[10px] text-muted-foreground uppercase tracking-widest">Optional Parameters</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-6">
                    ...
                    <div className="space-y-2">
                      <Label className="text-[10px] text-muted-foreground uppercase tracking-widest">Expiry Date (Optional)</Label>
                      <DateTimePicker value={expiresDate} onChange={setExpiresDate} />
                    </div>
                    <div className="space-y-2">
                  ...
                  <CardContent className="pt-8 space-y-8 relative z-10">
                    ...
                    <div>
                      <Textarea 
                        ...
                        className="w-full bg-transparent border-b-2 border-border focus:border-primary px-0 py-3 text-2xl md:text-3xl font-bold text-foreground outline-none transition-all placeholder:text-muted-foreground resize-none overflow-hidden leading-relaxed" 
                      />
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center h-8 w-full rounded-lg border border-input bg-muted/30 px-2.5 text-sm transition-colors focus-within:border-ring focus-within:ring-3 focus-within:ring-ring/50 overflow-hidden">
                        <span className="text-muted-foreground font-mono text-xs sm:text-sm select-none whitespace-nowrap pointer-events-none hidden sm:inline">
                          jharkhandexpress.com/article/
                        </span>
                        <span className="text-muted-foreground font-mono text-xs select-none whitespace-nowrap pointer-events-none sm:hidden">
                          .../
                        </span>
                        <input 
                          type="text" 
                          ...
                          className="w-full bg-transparent border-none outline-none font-mono text-sm placeholder:text-muted-foreground min-w-0"
                        />
                      </div>
                      <p className="text-[10px] text-muted-foreground italic">The AI will auto-generate an English URL friendly slug for SEO, but you can manually edit it.</p>
                    </div>
                    ...
                    <div className="space-y-3">
                      <Tabs value={htmlViewMode} onValueChange={(v) => setHtmlViewMode(v as any)} className="w-full">
                        ...
                        <TabsContent value="code" className="space-y-2">
                          {contentBlocks.map((block, idx) => (
                            <div key={idx} className="relative group animate-in fade-in">
                              ...
                              <Textarea 
                                ...
                                className="font-mono text-xs leading-relaxed bg-muted/30 resize-y min-h-25 shadow-inner" 
                              />
                              {contentBlocks.length > 1 && (
                                <Button 
                                  ...
                                  variant="destructive"
                                  size="icon"
                                  className="absolute top-5 right-2 w-6 h-6 opacity-0 group-hover:opacity-100 transition-opacity"
                                  onClick={() => {
                                    const newBlocks = [...contentBlocks];
                              ...
                              <div className="flex items-center justify-center gap-2 mt-4 mb-2 opacity-50 hover:opacity-100 transition-opacity">
                                <div className="h-px bg-border flex-1" />
                                <Button 
                                  type="button" 
                                  variant="outline" 
                                  size="sm" 
                                  className="text-[9px] uppercase h-6 px-2 rounded-full"
                                  onClick={() => {
                                    const newBlocks = [...contentBlocks];
                                    newBlocks.splice(idx + 1, 0, '<!-- wp:image -->\n<figure class="wp-block-image"><img src="IMAGE_URL" alt="" /></figure>\n<!-- /wp:image -->');
                                    setContentBlocks(newBlocks);
                                  }}
                                >
                          ...
                          >
                            <Plus className="w-4 h-4 mr-2" /> Add Text Block
                          </Button>
                        </TabsContent>
                        ...
                        <TabsContent value="preview">
                          <div className="w-full bg-background text-foreground rounded-md p-6 text-sm min-h-75 overflow-y-auto prose prose-sm max-w-none shadow-inner border border-border">
                            {unifiedHtmlPreview ? (
                               <div dangerouslySetInnerHTML={{ __html: unifiedHtmlPreview }} />
                            ) : (
                              <div className="h-full flex flex-col items-center justify-center text-muted-foreground italic mt-20">
                                <Eye className="w-8 h-8 mb-2 opacity-50" />
                                <p>Output preview will render here.</p>
                              </div>
                            )}
                          </div>
                        </TabsContent>
                    ...
                    <div className="flex items-center gap-2 p-4 bg-muted/30 border border-border rounded-lg mt-4">
                      <Checkbox 
                        id="pushBroadcast"
                        name="pushBroadcast"
                        checked={isPushBroadcast} 
                        onCheckedChange={(c) => setIsPushBroadcast(!!c)} 
                        value="true"
                      />
                      <Label htmlFor="pushBroadcast" className="text-sm font-semibold flex items-center gap-2 cursor-pointer">
                        <Bell className="w-4 h-4 text-brand-red" />
                        Broadcast via Push Notification
                      </Label>
                      <p className="text-xs text-muted-foreground ml-auto hidden sm:block">Sends a native push alert to all subscribers.</p>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 mt-6">
                      <Button
                        ...
                        size="lg"
                        disabled={isPending || isUploading}
                        className="flex-1 py-6 relative overflow-hidden"
                      >
                        {youtubeUploading && (
                        ...
                        <div className="relative z-10 flex items-center justify-center">
                          {isPending || isUploading || isCompressing || isUploadingMedia ? (
                            <><Loader2 className="w-5 h-5 animate-spin mr-2" /> {isCompressing ? 'COMPRESSING IMAGES...' : youtubeUploading ? `UPLOADING TO YOUTUBE ${youtubeUploadProgress}%` : isUploadingMedia ? 'UPLOADING MEDIA...' : 'COMMITTING...'}</>
                          ) : (
