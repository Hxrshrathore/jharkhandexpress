...
export async function editFeedItem(prevState: FormState, formData: FormData): Promise<FormState> {
  ...
  if (!id || !title || !content_html) {
    return { success: false, message: 'All required core fields (including ID) must be provided.' };
  }

  // Receive final media URLs and IDs from the client
  ...
  try {
    ...
    // Replace custom UI placeholders
    ...
    // Weave Youtube Videos, then Images
    let unusedGalleryUrls = [...gallery_urls];
    let unusedYoutubeIds = youtubeVideoIds.filter(yid => yid !== featured_video_id);
    ...
    await sql`
      UPDATE articles SET
        ...
        video_gallery = ${video_gallery}::jsonb,
        ...
        youtube_video_id = ${featured_video_id},
        youtube_status = ${youtube_status},
        expires_at = ${expires_at},
        featured_video_timestamp = ${featured_video_timestamp}
      WHERE guid::text = ${id} OR id::text = ${id}
    `;
